...
public class SentenceAnalysisForm extends CustomComponent {
    ...
    public void init() {
        ...
        sentenceUnitLayout.setVisible(false);
        ...
        sentenceAverageLengthHistogram.addValueChangeListener(event -> sentenceUnitLayout.setVisible(!sentenceUnitLayout.isVisible()));
        
        sentenceRegularExpression.setCaption(provider.getProperty("label.regular.expression"));
        sentenceRegularExpression.addValidator(regExpValidator);
        sentenceRegularExpression.addStyleName(ValoTheme.TEXTFIELD_TINY);
        ...
        FormLayout frm = new MFormLayout(sentenceRegularExpression);
        VerticalLayout layout = new MVerticalLayout()
                .withStyleName(ChronoTheme.COMPACT_FORM)
                .with(frm, sentenceAverageLengthHistogram, sentenceUnitLayout);
